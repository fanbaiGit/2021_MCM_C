...
def loadDataSet(fileName):  # 输出dataArr(m*n),labelArr(1*m)其中m为数据集的个数
    ...
    for line in fr.readlines():
        ...
        dataMat.append([float(lineArr[-1]), float(lineArr[-2])])  # 数组矩阵
        labelMat.append(1 if float(lineArr[-1]) < -123.2 and float(lineArr[-2]) > 48.2 else -1)  # 标签
...
def testRbf(k1=1.3):  # 给定输入参数K1
    # 测试训练集上的准确率
    dataArr, labelArr = loadDataSet('csv/testSetRBF.csv')  # 导入数据作为训练集
    ...
    for i in range(m):  # 开始分类，是函数的核心
        ...
        if sign(predict) != sign(labelArr[i]):
            errorCount += 1  # 利用符号判断类别
        ### sign（a）为符号函数：若a>0则输出1，若a<0则输出-1.###
    print("the training error rate is: %f" % (float(errorCount) / m))
    # 2、测试测试集上的准确率
    dataArr, labelArr = loadDataSet('csv/testSetRBF2.csv')
    errorCount = 0
...
def plotBestFit(weights):
    ...
    dataMat, labelMat = loadDataSet('csv/PositiveID.csv')
...
def main():
    t1 = time()
    dataArr, labelArr = loadDataSet('csv/testSetRBF.csv')
